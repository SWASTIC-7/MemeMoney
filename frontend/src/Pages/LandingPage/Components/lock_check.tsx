...
const TokenSaleValidator: React.FC = () => {
  ...
  const getUserInfo = async () => {
    ...
    try {
      const [currentMilestoneIndex, lastSaleTime, totalSold, isPostGraduation, nextMilestone, timeUntilNextSale] = 
        await contract.getUserSaleInfo(account);
      
      const milestones = await contract.getRemainingMilestones(account);
      const maxAmount = await contract.getMaxSaleAmount(parseEther(totalTokens));
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-white rounded-2xl shadow-xl p-8">
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Token Sale Validator</h1>
          ...
          <div className="bg-gray-50 rounded-lg p-6 mb-6">
            ...
            <div className="grid md:grid-cols-3 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Graduation Progress (%)</label>
                <input
                  type="number"
                  value={graduationProgress}
                  onChange={(e) => setGraduationProgress(Number(e.target.value))}
                  min="0"
                  max="100"
                  className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                />
          ...
          {message && (
            <div className={`rounded-lg p-4 mb-6 flex items-center ${
              ...
              {messageType === 'success' ? <CheckCircle className="mr-2" size={20} /> :
               messageType === 'error' ? <AlertCircle className="mr-2" size={20} /> :
          ...
          {userInfo && (
            <div className="bg-gray-50 rounded-lg p-6">
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                <div className="bg-white p-4 rounded-lg">
                  ...
                  <p className="text-2xl font-bold text-blue-600">
                    {userInfo.isPostGraduation ? 'Post-Graduation' : `Milestone ${userInfo.currentMilestoneIndex + 1}`}
                  </p>
                </div>
                <div className="bg-white p-4 rounded-lg">
                  <h3 className="font-medium text-gray-700">Total Sold</h3>
                  <p className="text-2xl font-bold text-green-600">{userInfo.totalSold}</p>
                </div>
                <div className="bg-white p-4 rounded-lg">
                  <h3 className="font-medium text-gray-700">Max Sale Amount (2%)</h3>
                  <p className="text-2xl font-bold text-purple-600">{maxSaleAmount}</p>
                </div>
                <div className="bg-white p-4 rounded-lg">
                  <h3 className="font-medium text-gray-700">Next Milestone</h3>
                  <p className="text-2xl font-bold text-orange-600">{userInfo.nextMilestone}%</p>
                </div>
                <div className="bg-white p-4 rounded-lg">
                  <h3 className="font-medium text-gray-700">Next Sale Available</h3>
                ...
                <div className="bg-white p-4 rounded-lg">
                  ...
                  <p className={`text-2xl font-bold ${canSellTokens ? 'text-green-600' : 'text-red-600'}`}>
              ...
              {remainingMilestones.length > 0 && (
                <div className="mt-4">
                  <h3 className="font-medium text-gray-700 mb-2">Remaining Milestones</h3>
